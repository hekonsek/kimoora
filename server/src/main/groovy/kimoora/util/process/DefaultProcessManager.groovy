...
class DefaultProcessManager extends ExecutorBasedProcessManager {
    ...
    private final SudoResolver sudoResolver

    DefaultProcessManager(SudoResolver sudoResolver) {
        this.sudoResolver = sudoResolver
    }
    ...
    List<String> execute(Command command) {
        ...
        try {
            ...
            def output = IOUtils.readLines(process.getInputStream())
